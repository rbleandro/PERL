...
if ($mount_pt eq ""){
   ...
   $mount_msgs = `sudo mount -t cifs -o user\=\"canparnt\\gl_ftp_user",password\=C\@np\@r12 \/\/cprhqvfs4.canpar.com\/AppData\/MPR_DATA /opt/sap/bcp_data/mpr_data/gl_extract -o noperm 2>&1`;
...
bcp mpr_data..gl_national_groups in /opt/sap/bcp_data/mpr_data/gl_extract/gl_groupings.csv -Usa -S$prodserver -P\`/opt/sap/cron_scripts/getpass.pl sa\` -c -t","  -r"\r\n" -b1`;
}else{
...
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
Subject: Error - GL Groups Upload procedure
...
$mv_msg = `cp /opt/sap/bcp_data/mpr_data/gl_extract/gl_groupings.csv /opt/sap/bcp_data/mpr_data/gl_extract_bkp/ 2>&1`;
...
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
Subject: Error - copying gl_groupings.csv to gl_extract_bkp
...
$del_msg = `sudo rm -f /opt/sap/bcp_data/mpr_data/gl_extract/gl_groupings.csv`;

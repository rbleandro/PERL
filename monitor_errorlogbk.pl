...
#Usage Restrictions
if ($#ARGV != 0){
   print "Usage: monitor_errorlog.pl CPDB1 \n";
...
Subject: CPDB1 Error Alert

Errors Found In CPDB1 Errorlog!!!
...
print "*********\nMail Sent To DBAs\@canpar.com\n*********\n";
   ...
   if(/$currDate/){
      if (/$currHour\:$currMin\:/){
         if((/webtest/ || /Error/ || /sleeping/i || /critically/i || /failed/i || /degradation/i || /deadlock/i || /stack trace/i || /fatal/i || /critical/i || /severity\: [10..26]/i)&&(!/1608/)){
	    $firstLine = $_;
            $getNextLine = 1;
         }
      }
...
`mail -s "CPDB1 IS DOWN!!!" \`cat /opt/sybase/sybmail/SYB_DEV_GROUP\` <<EOF
\*\*\*\!\!\!Server Is Down\!\!\!\*\*\*
...
`echo 0 > /tmp/bsrvcnt`;
   ...
   if ($BINITSRVCNT == 2){
      ...
      `/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com,CANPARDBASybaseMobileAlerts\@canpar.com
Subject: CPDB1 Backup Server IS DOWN!!!

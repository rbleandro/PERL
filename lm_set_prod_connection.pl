...
print "iq_set_connection:status: $switch StartTime: $currTime, Hour: $startHour, Min: $startMin\n";

if ($switch =~ /suspend/){#if input asks for suspend the connection
   $sqlerror = `. /opt/sybase/SYBASE.sh
isql -Usa -w200 -Ps9b2s3 -Ssybrep2 -b -Jroman8 <<EOF 2>&1
suspend connection to $prodserver.lm_stage   
go
exit
EOF
`;
   ...
   $sqlerror = `. /opt/sybase/SYBASE.sh
isql -Usa -w200 -Ps9b2s3 -Ssybrep2 -b -Jroman8 <<EOF 2>&1
resume connection to $prodserver.lm_stage   
go
exit
EOF
`;

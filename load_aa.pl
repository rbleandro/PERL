...
#
#25/02/11       Ahsan Ahmed     Added code to run the atu cmf_data update for 
#                               testing 
###################################################################################
...
print "\n###Killing any users still logged into cdpvkm###\n";
`/opt/sybase/cron_scripts/kill_processes.pl $server cdpvkm`;
...
$loadError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -S$server <<EOF 2>&1
load database cdpvkm from "/opt/sybase/db_backups/stripe11/cdpvkm.dmp"
go
select "Load of cdpvkm finished at "+ convert(varchar,getdate(),109)
go
exit
EOF
`;
...
use cmf_data
go
update dbo.testing set attachment_file_path='C:\\temp'
go
exit
EOF
`;

`/opt/sybase/cron_scripts/misc_update_cmf_data.pl > /opt/sybase/cron_scripts/cron_logs/misc_update_cmf_data.log`;
print $onlineError."\n";
die;

...
$my_pid = getppid();
$isProcessRunning =`ps -ef|grep sybase|grep mpr_data_lm_load_master_record.pl|grep -v grep|grep -v $my_pid|grep -v "vim mpr_data_lm_load_master_record.pl"|grep -v "less mpr_data_lm_load_master_record.pl"`;
...
Subject: Errors - mpr_data_lm_load_master_record at $finTime

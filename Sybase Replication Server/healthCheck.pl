...
#July 26 2019	Rafael Leandro	1.Obfuscated password for security reasons.
#								2.Changed the script to only check the database that have actual subscriptions created for them (this avoids unecessary checks and mail alerts).
#								3.Included check for the DR site.
#Aug 8 2019		Rafael Leandro	1.Added sqm block validation. Sometimes the ticket takes a bit to be replicated. The alert will now only fire if the SQM is actually not moving. This validation was not added for IQ.
#								2.Added error handling for isql connections
#configuring parameter for mail recipients
my $dba = $ARGV[1];
if (defined $dba) {
    $mail=$dba;
} else {
...
use master
go
select name from master..sysdatabases where name not in ('master','sybmgmtdb','model','sybsecurity','sybsystemdb','sybsystemprocs','lm_stage') and name not like 'temp%' and dsname='CPDB2' order by name
go
exit
EOF
`;
...
use master
go
select name from master..sysdatabases where name not in ('master','sybmgmtdb','model','sybsecurity','sybsystemdb','sybsystemprocs','lm_stage') and name not like 'temp%' and dsname='CPDB4' order by name
go
exit
EOF
`;

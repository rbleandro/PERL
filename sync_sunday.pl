...
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -S$prodserver -n -b -s'|'<<EOF 2>&1
use cpscan
go
...
use cpiq1
go
select count(*) from tttl_ev_event
go
exit
EOF
`;
...
use cpiq1
go
select count(*) from tttl_ma_manifestt_2010
go
exit
EOF
`;
...
use rev_hist
go
select count(*) from cwparcel_live
go
exit
EOF
`;

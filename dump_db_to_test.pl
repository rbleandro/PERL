...
die;
	    ...
	    #Just load it to test as a weekly load
           ## $sshError_test = `ssh CPSYBTEST.canpar.com /opt/sybase/cron_scripts/load_db.pl CPSYBTEST cpscan $startDay $startHour $startMin`;
           ## print $sshError_test."\n\n";

            die "The $database has been dumped only\n";
            }
	    print "Starting load on test server... \n";
	    $sshError_test = `ssh CPSYBTEST.canpar.com /opt/sybase/cron_scripts/load_db.pl CPSYBTEST cpscan $startDay $startHour $startMin`;
	    print $sshError_test."\n\n";

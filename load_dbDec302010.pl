...
resume connection to $server.cpscan
go
exit
EOF
`;
...
print $onlineError."\n";
   ...
   $resumeError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -SSYBREP1 <<EOF 2>&1
resume connection to $server.rev_hist
go
exit
EOF
`;
...
print $onlineError."\n";
   ...
   $resumeError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -SSYBREP1 <<EOF 2>&1
resume connection to $server.cmf_data
go
exit
EOF
`;
...
$loadError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -S$server <<EOF 2>&1
load database eput from "/opt/sybase/db_backups/stripe14/eput.dmp1"
go
...
print $onlineError."\n";
   ...
   $resumeError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -SSYBREP1 <<EOF 2>&1
resume connection to $server.liberty_db
go
exit
EOF
`;

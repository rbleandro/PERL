...
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -Shqvsybrep2 -w300 <<EOF 2>&1
suspend connection to $standbyserver.cmf_data_lm with nowait
go
drop connection to $standbyserver.cmf_data_lm
go
wait for delay '00:00:15'
go
create connection to $standbyserver.cmf_data_lm
set error class to rs_sqlserver_error_class
set function string class to rs_sqlserver_function_class
set username to cmf_data_lm_maint
set password to sybase
set db_packet_size to '2048'
with log transfer on
as standby for LDS.cmf_data_lm
use dump marker
go
exit
EOF
...
$dumpError = `. /opt/sybase/SYBASE.sh
isql -Ubackup -P\`/opt/sybase/cron_scripts/getpass.pl backup\` -S$prodserver <<EOF 2>&1
dump database cmf_data_lm to "/opt/sybase/db_backups/stripe11/cmf_data_lm.dmp"
go
select "Dump of cmf_data_lm finished at "+ convert(varchar,getdate(),109)
go
exit
EOF
`;
...
if($dumpError =~ /complete/){
   print "Dump was successful, starting scp process at ".localtime()."\n\n";
      $currTime = localtime();
      $scpError=`scp -p /opt/sybase/db_backups/stripe11/cmf_data_lm.dmp sybase\@$standbyserver:/opt/sybase/db_backups/stripe11/
      `;
         print $scpError."\n";
         ...
         $sshTestError = `ssh $standbyserver '/opt/sybase/cron_scripts/scp_load_db.pl cmf_data_lm'`;
...
print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh $standbyserver /opt/sybase/cron_scripts/load_db.pl $standbyserver cmf_data_lm $startDay $startHour $startMin`;
            print $sshError."\n\n";
...
$sqlError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -Shqvsybrep2 -w300 <<EOF 2>&1
suspend connection to $standbyserver.lmscan
go
drop connection to $standbyserver.lmscan
go
wait for delay '00:00:05'
go
create connection to $standbyserver.lmscan
set error class to rs_sqlserver_error_class
set function string class to rs_sqlserver_function_class
set username to lmscan_maint
set password to sybase
set db_packet_size to '2048'
set parallel_dsi to 'on'
set dsi_sqt_max_cache_size to '90000000'
with log transfer on
as standby for LDS.lmscan
use dump marker
go
exit
EOF
...
$dumpError = `. /opt/sybase/SYBASE.sh
isql -Ubackup -P\`/opt/sybase/cron_scripts/getpass.pl backup\` -S$prodserver <<EOF 2>&1
dump database lmscan to "/opt/sybase/db_backups/stripe11/lmscan.dmp"
with compression=100
go
select "Dump of lmscan finished at "+ convert(varchar,getdate(),109)
go
exit
EOF
`;
...
if($dumpError =~ /complete/){
   print "Dump was successful, starting scp process at ".localtime()."\n\n";
      $currTime = localtime();
      $scpError=`scp -p /opt/sybase/db_backups/stripe11/lmscan.dmp sybase\@10.3.1.71:/opt/sybase/db_backups/stripe11/
      `;
         print $scpError."\n";
   if($scpError =~ /no|not/i){
      print "Errors may have occurred during scp...\n\n";
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
Subject: ERROR - SCP FOR DATABASE DUMP: $database
...
die;
         ...
         #$sshTestError = `ssh $standbyserver '/opt/sybase/cron_scripts/scp_load_db.pl lmscan'`;
...
print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh 10.3.1.71 /opt/sybase/cron_scripts/load_db.pl $standbyserver lmscan $startDay $startHour $startMin`;
            print $sshError."\n\n";
...
if ($database eq "linehaul_data"){

#***************Suspending replication and preparing to resync************
$sqlError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -Shqvsybrep2 -w300 <<EOF 2>&1
suspend connection to $standbyserver.linehaul_data
go
drop connection to $standbyserver.linehaul_data
go
wait for delay '00:00:15'
go
create connection to $standbyserver.linehaul_data
set error class to rs_sqlserver_error_class
set function string class to rs_sqlserver_function_class
set username to linehaul_data_maint
set password to sybase
set db_packet_size to '2048'
with log transfer on
as standby for LDS.linehaul_data
use dump marker
go
exit
EOF
...
print "\n***Removing existing dumps from directories...***\n";
`rm /opt/sybase/db_backups/stripe11/linehaul_data.dmp`;

print "***Initiating Dump At:".localtime()."***\n";
$dumpError = `. /opt/sybase/SYBASE.sh
isql -Ubackup -P\`/opt/sybase/cron_scripts/getpass.pl backup\` -S$prodserver <<EOF 2>&1
dump database linehaul_data to "/opt/sybase/db_backups/stripe11/linehaul_data.dmp"
go
select "Dump of linehaul_data finished at "+ convert(varchar,getdate(),109)
go
exit
EOF
`;
...
if($dumpError =~ /complete/){
   print "Dump was successful, starting scp process at ".localtime()."\n\n";
      $currTime = localtime();
      $scpError=`scp -p /opt/sybase/db_backups/stripe11/linehaul_data.dmp sybase\@$standbyserver:/opt/sybase/db_backups/stripe11/
      `;
         print $scpError."\n";
   if($scpError =~ /no|not/i){
      print "Errors may have occurred during scp...\n\n";
...
die;
         ...
         $sshTestError = `ssh $standbyserver '/opt/sybase/cron_scripts/scp_load_db.pl linehaul_data'`;
...
print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh $standbyserver /opt/sybase/cron_scripts/load_db.pl $standbyserver linehaul_data $startDay $startHour $startMin`;
            print $sshError."\n\n";
 ...
 Following status was received after linehaul_data scp that started on $currTime
 $scpError
  EOF
     `;
     }
...
$sqlError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -Shqvsybrep2 -w300 <<EOF 2>&1
suspend connection to $standbyserver.rev_hist_lm
go
drop connection to $standbyserver.rev_hist_lm
go
wait for delay '00:00:15'
go
create connection to $standbyserver.rev_hist_lm
set error class to rs_sqlserver_error_class
set function string class to rs_sqlserver_function_class
set username to rev_hist_lm_maint
set password to sybase
set db_packet_size to '2048'
with log transfer on
as standby for LDS.rev_hist_lm
use dump marker
go
exit
EOF
...
$dumpError = `. /opt/sybase/SYBASE.sh
isql -Ubackup -P\`/opt/sybase/cron_scripts/getpass.pl backup\` -S$prodserver <<EOF 2>&1
dump database rev_hist_lm to "/opt/sybase/db_backups/stripe11/rev_hist_lm.dmp"
go
select "Dump of rev_hist_lm finished at "+ convert(varchar,getdate(),109)
go
exit
EOF
`;
...
if($dumpError =~ /complete/){
   print "Dump was successful, starting scp process at ".localtime()."\n\n";
      $currTime = localtime();
      $scpError=`scp -p /opt/sybase/db_backups/stripe11/rev_hist_lm.dmp sybase\@$standbyserver:/opt/sybase/db_backups/stripe11/
      `;
         print $scpError."\n";
         ...
         $sshTestError = `ssh $standbyserver '/opt/sybase/cron_scripts/scp_load_db.pl rev_hist_lm'`;
...
print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh $standbyserver /opt/sybase/cron_scripts/load_db.pl $standbyserver rev_hist_lm $startDay $startHour $startMin`;
            print $sshError."\n\n";
...
#############################
#Changing CPDB2 to 10.3.1.71
#To setup new server
#############################

$standbyserver = "10.3.1.71";
...
die;
	    ...
	    #Just load it to test as a weekly load
           ...
           ## print $sshError_test."\n\n";
            ...
            print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh $standbyserver /opt/sybase/cron_scripts/load_db.pl CPDB2 cpscan $startDay $startHour $startMin`;
            print $sshError."\n\n";
...
#wait for delay '00:00:15'
#go
#create connection to $standbyserver.mpr_data
#set error class to rs_sqlserver_error_class
#set function string class to rs_sqlserver_function_class
#set username to mpr_data_maint
#set password to sybase
#set db_packet_size to '2048'
#with log transfer on
#as standby for LDS.mpr_data
#use dump marker
#go
#exit
#EOF
...
`;
            ...
            print "Starting load on $standbyserver through ssh...\n\n";
            $sshError = `ssh $standbyserver /opt/sybase/cron_scripts/load_db.pl CPDB2 mpr_data $startDay $startHour $startMin`;
            print $sshError."\n\n";
            if($sshError =~ /Failed/){
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
...
$sqlError = `. /opt/sybase/SYBASE.sh
isql -Usa -P\`/opt/sybase/cron_scripts/getpass.pl sa\` -Shqvsybrep1 -w300 <<EOF 2>&1
suspend connection to $standbyserver.canship_webdb
go
drop connection to $standbyserver.canship_webdb
go
wait for delay '00:00:15'
go
create connection to $standbyserver.canship_webdb
set error class to rs_sqlserver_error_class
set function string class to rs_sqlserver_function_class
set username to canship_webdb_maint
set password to sybase
set db_packet_size to '2048'
with log transfer on
as standby for LDS.canship_webdb
use dump marker
go
exit
EOF

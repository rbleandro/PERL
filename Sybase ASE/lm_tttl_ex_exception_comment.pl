...
$reset_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run -mode RESET \-wait loomis lm_tttl_ex_exception_comment 2>&1`;
$gen_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run \-wait loomis lm_tttl_ex_exception_comment 2>&1`;
$job_info=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-logdetail loomis lm_tttl_ex_exception_comment 2>&1`;

#print "Generation Msg: $gen_msg \n Job Info: $job_info \n";

if ($job_info =~ /Finished with warnings/ || $job_info =~ /aborted/ || $job_info =~ /Not connected/){
   if ($job_info !~ /0 Rows affected/){
	print "Emailing Now \n";
	...
	print MAIL "Subject: lm_tttl_ex_exception_comment Messages\n\n";

...
$reset_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run \-warn 1 -mode RESET \-wait loomis tttl_ex_pms_seq 2>&1`;
$gen_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run \-warn 1000000 \-wait loomis tttl_ex_pms_seq 2>&1`;
$job_info=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-logdetail loomis tttl_ex_pms_seq 2>&1`;
...
if ($job_info =~ /Finished with warnings/ || $job_info =~ /aborted/ || $job_info =~ /Error running job/){
	...
	if ( $job_info =~ /No row was found/) { die;} # Ingore garbage data update issues
print "Emailing Now \n";
...
print MAIL "Subject: Loomis HUB_Transfers Exception Comments tttl_ex_pms_seq Messages\n\n";

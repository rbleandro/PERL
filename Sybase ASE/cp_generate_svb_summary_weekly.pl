...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep cp_generate_svb_summary_weekly.pl|grep -v grep|grep -v $my_pid|grep -v "vim cp_generate_svb_summary_weekly.pl"|grep -v "less cp_generate_svb_summary_weekly.pl"`;
...
To:$mail\@canpar.com
Subject: Error: cp_generate_svb_summary_weekly at $finTime

...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep cp_maintain_svb_summary.pl|grep -v grep|grep -v $my_pid|grep -v "vim cp_maintain_svb_summary.pl"|grep -v "less cp_maintain_svb_summary.pl"`;
...
To:$mail\@canpar.com
Subject: Error: cp_maintain_svb_summary at $finTime

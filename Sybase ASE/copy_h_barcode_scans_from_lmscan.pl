...
-------------------------------------------- DELETING RECORDS BELONGING TO A BARCODE THAT WAS INSERTED INTO lmscan..tttl_ma_barcode ----------------------------------

select
ev.alt_barcode as reference_num, ev.scan_time_date, ev.conv_time_date, ev.employee_num,ev.BTACC
into #delete_this
...
select
ev.alt_barcode as reference_num, ev.scan_time_date, ev.conv_time_date, ev.employee_num,ev.BTACC,ev.alt_shipper_num
into #delete_loomis_pieces
from cpscan..tttl_ev_event_hospital ev
where ev.BTACC = 'LOOMIS'
...
while \@count > 0
begin
    update top 1000 lmscan..tttl_ev_event_rawbc set company_code = 'LM'
...
delete cpscan..tttl_ev_event_hospital
from #delete_loomis_pieces m
join cpscan..tttl_ev_event_hospital ev on m.reference_num = ev.alt_barcode and m.conv_time_date=ev.conv_time_date and m.employee_num=ev.employee_num and m.alt_shipper_num = ev.alt_shipper_num

--------------------------------------------------------------------------------------------------------------------------

-------------------------------------------- CLEANING UP THE HOSPITAL OF BILLED MDS PIECES ----------------------------------

select distinct e.alt_barcode as reference_num, e.alt_shipper_num,e.employee_num,e.conv_time_date , e.BTACC,e.scan_time_date
into #delete_mds_billed_pieces
from cpscan..tttl_ev_event_hospital e
inner join cpscan..tttl_ev_event_extended ee on ee.alt_barcode = e.alt_barcode 
inner join rev_hist..bcxref b on b.service_type = ee.service_type and b.shipper_num = ee.shipper_num and b.reference_num = ee.reference_num
where 1=1
and ee.service_type ='H'
plan '(use optgoal allrows_oltp)'

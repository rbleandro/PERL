...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep mobitrax_jobs_manifest_populate.pl|grep -v grep|grep -v $my_pid|grep -v "vim mobitrax_jobs_manifest_populate.pl"|grep -v "less mobitrax_jobs_manifest_populate.pl"`;
...
use cpscan
go
exec cpscan..mobitrax_jobs_manifest_populate
go
exit
EOF
`;
...
To:$mail\@canpar.com
Subject: Error: mobitrax_jobs_manifest_populate at $finTime

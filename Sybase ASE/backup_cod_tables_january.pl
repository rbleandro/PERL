...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep backup_cod_tables_january.pl|grep -v grep|grep -v $my_pid|grep -v "vim backup_cod_tables_january.pl"|grep -v "less backup_cod_tables_january.pl"`;
...
To:$mail\@canpar.com
Subject: Error: backup_cod_tables_january at $finTime

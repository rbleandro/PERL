...
`elinks \"http\:\/\/cprhqvprtlstg.canpar.com\/sp\/batchParcelEvaluation.do\?tokenId\=canpar\&token\=M985NRKHVKWuet26QvsMkQbP6mY\=\" \> \/tmp\/out.out 2>&1`;
sleep(20); 

`cat /tmp/out.out`;

while (1==1){
unless (-e "/tmp/svp_p_completed"){ 
sleep(5);
$monitorOutput = `/opt/sap/cron_scripts/svp_parcel_url_monitor.pl \"$date_flag\"`;

...
$reset_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run -mode RESET \-wait loomis lm_PictureDataCapture 2>&1`;
$gen_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run \-wait \-warn 10000 loomis lm_PictureDataCapture 2>&1`;
$job_info=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-logdetail loomis lm_PictureDataCapture 2>&1`;

#print "Generation Msg: $gen_msg \n Job Info: $job_info \n";

if ($job_info =~ /Finished with warnings/ || $job_info =~ /aborted/ || $job_info =~ /Not connected/){
 if ($job_info =~ /0 Rows affected/){die;} # If no rows affected, I don't want to know}
print "Emailing Now \n";
...
print MAIL "Subject: Canpar lm_PictureDataCapture Messages\n\n";

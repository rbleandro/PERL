...
if ($mount_pt eq ""){
   ...
   $mount_msgs = `sudo mount -t cifs -o user\=\"canparnt\\gl_ftp_user",password\=C\@np\@r12 \/\/cprhqvfs4\/AppData\/MPR_DATA_LM /opt/sap/bcp_data/mpr_data_lm/gl_extract -o noperm 2>&1`;
...
bcp mpr_data_lm..gl_national_groups in /opt/sap/bcp_data/mpr_data_lm/gl_extract/gl_groupings_lm.csv -Ucronmpr -S$prodserver -P\`/opt/sap/cron_scripts/getpass.pl cronmpr\` -c -t","  -r"\r\n" -b1`;
}else{
...
Subject: Error - bcp of gl_groupings for Loomis
...
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
Subject: Error - GL Groups Upload procedure for Loomis

Following status was received during mpr_gl_groups_upload_data that started on $currTime
$sqlError
EOF
`;
...
$mv_msg = `cp /opt/sap/bcp_data/mpr_data_lm/gl_extract/gl_groupings_lm.csv /opt/sap/bcp_data/mpr_data_lm/gl_extract_bkp/ 2>&1`;
...
`/usr/sbin/sendmail -t -i <<EOF
To: CANPARDatabaseAdministratorsStaffList\@canpar.com
Subject: Error - copying gl_groupings.csv to gl_extract_bkp for Loomis

Following status was received during gl_groupings.csv file copy to gl_extract_bkp that started on $currTime
$mv_msg
EOF
`;
...
$del_msg = `sudo rm -f /opt/sap/bcp_data/mpr_data_lm/gl_extract/gl_groupings_lm.csv`;

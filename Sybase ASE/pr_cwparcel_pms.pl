...
$reset_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run -mode RESET \-wait loomis pr_cwparcel_pms 2>&1`;
$gen_msg=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-run \-wait loomis pr_cwparcel_pms 2>&1`;
$job_info=`C\:\/IBM\/InformationServer\/Server\/DSEngine\/bin\/dsjob.exe \-logdetail loomis pr_cwparcel_pms 2>&1`;
...
if ($job_info =~ /Finished with warnings/ || $job_info =~ /abort/i || $job_info =~ /Error running job/){
 if ($job_info !~ /No row was found/){
	...
	print MAIL "Subject: rev_hist_lm\\pr_cwparcel_pms Messages\n\n";

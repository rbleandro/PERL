...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep document_process_breaks.pl|grep -v grep|grep -v $my_pid|grep -v "vim document_process_breaks.pl"|grep -v "less document_process_breaks.pl"`;
...
use cmf_data
go
exec hub_db..document_process_breaks 
go
exit
EOF
`;
...
To:$mail\@canpar.com
Subject: Error: document_process_breaks at $finTime

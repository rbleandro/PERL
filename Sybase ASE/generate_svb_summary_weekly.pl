...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep generate_svb_summary_weekly.pl|grep -v grep|grep -v $my_pid|grep -v "vim generate_svb_summary_weekly.pl"|grep -v "less generate_svb_summary_weekly.pl"`;
...
To:$mail\@canpar.com
Subject: Error: generate_svb_summary_weekly at $finTime

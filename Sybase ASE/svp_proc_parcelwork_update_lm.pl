...
update svp_lm..svp_parcel set inserted_on_cons =dateadd(millisecond, -datepart(millisecond, inserted_on_cons), inserted_on_cons) ,
updated_on_cons =dateadd(millisecond, -datepart(millisecond, updated_on_cons), updated_on_cons)
where first_scan_date > convert(varchar(12),dateadd(dd,-100,getDate())) and made_service=''  and
first_scan_date <>'1900/01/01 00:00:00.0'

...
my $my_pid = getppid();
my $isProcessRunning =`ps -ef|grep sybase|grep lm_generate_svb_summary_weekly.pl|grep -v grep|grep -v $my_pid|grep -v "vim lm_generate_svb_summary_weekly.pl"|grep -v "less lm_generate_svb_summary_weekly.pl"`;
...
To:$mail\@canpar.com
Subject: Error: lm_generate_svb_summary_weekly at $finTime

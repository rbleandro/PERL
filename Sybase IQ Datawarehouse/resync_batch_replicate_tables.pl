...
#if (1==2){
$my_pid = getppid();
$isProcessRunning =`ps -ef|grep sybase|grep resync_batch_replicate_tables.pl|grep -v sh|grep -v grep|grep -v $my_pid|grep -v "vim resync_batch_replicate_tables.pl"|grep -v "less resync_batch_replicate_tables.pl"`;
...
Subject: Canpar - resync_batch_replicate_tables
